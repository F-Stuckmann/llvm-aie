...
; REQUIRES: asserts
;
; The post-ISel AIEOuterLoopStageSplit pass reports each OLP movement candidate
; (the steady-state latch carrying the pipeliner's success metadata) it finds in
; the skip-split CFG skeleton. A function with no pipelined loop reports nothing.
;
...
; A function without a pipelinable loop must not be reported as a candidate.
define void @no_loop(ptr %p) {
entry:
