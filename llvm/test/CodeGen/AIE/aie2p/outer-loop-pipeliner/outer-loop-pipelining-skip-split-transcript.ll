...
; Pins the full post-pass IR for a kernel whose prologue mul feeds the inner
; accumulator, verifying no instruction is dropped, added, or reordered.

; CHECK-LABEL: define void @skip_split_full(ptr noalias %a, ptr noalias %b, ptr noalias %c, i32 %N, i32 %M) {
; CHECK-NEXT: entry:
...
; CHECK-NEXT: %.top = mul i32 %v1.steady.top, %v0.steady.top
...
; CHECK-NEXT: %lsr.iv.steady = phi i32 [ %lsr.iv.next.steady, %steady.stage1.inner.inner.header ], [ %.phi, %steady.stage1.top ]
; CHECK-NEXT: %inner.cond.steady = call i1 @llvm.loop.decrement.i32(i32 1)
; CHECK-NEXT: %lsr.iv.next.steady = add i32 %lsr.iv.steady, %v0.steady.phi
...
; CHECK-NEXT: %.bottom = mul i32 %v1.steady.bottom, %v0.steady.bottom
...
; CHECK-NEXT: %lsr.iv.lastiter = phi i32 [ %lsr.iv.next.lastiter, %lastiter.stage1.inner.inner.header ], [ %.bottom, %lastiter.stage1.top ]
; CHECK-NEXT: %inner.cond.lastiter = call i1 @llvm.loop.decrement.i32(i32 1)
; CHECK-NEXT: %lsr.iv.next.lastiter = add i32 %lsr.iv.lastiter, %v0.steady.bottom
...
define void @skip_split_full(ptr noalias %a, ptr noalias %b, ptr noalias %c,
                              i32 %N, i32 %M) {
entry:
...
inner.header:
  %acc = phi i32 [ %init, %outer.header ], [ %acc.next, %inner.header ]
