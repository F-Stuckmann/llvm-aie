...
; Steady-state latch: GEPs are NOT here (lifted), hardware loop counter replaces software iv
; CHECK: steady.latch:
; CHECK:   store i32
...
; CHECK:   %a.ptr.steady = phi ptr [ %a.ptr.next.steady, %steady.latch ], [ %a, %steady.preheader ]
; CHECK:   %c.ptr.next.steady = getelementptr inbounds i8, ptr %c.ptr.steady, i64 64
; CHECK:   br label %steady.inner.header

; Steady-state latch: a.ptr.next stays here (inner loop dependency)
; CHECK: steady.latch:
