...
void AIEOuterLoopPipeliner::deleteOrigNest(const LoopStructure &OrigLS) const {
  ...
  // todo: In the shared-exit topology the exit block's LCSSA PHIs are rerouted
  // onto the steady-state latch (see swapInClonedNest) while the now-dead
  // original latch still branches to the exit. Plain DeleteDeadBlocks would
  // then call exit->removePredecessor(origLatch) and assert in
  // PHINode::removeIncomingValue, because the PHI no longer lists the original
  // latch as an incoming block. Detach the dead blocks by hand first
...
// Adjust the outer loop trip count from N to N-1 using the pre-computed
// LS.bound() (populated by tryAdjustLoopBound). The unified formula is:
//   NewLimit = Limit - Step
