//===-- AddAssume.cpp - Top-level interface for AIE -----------------*- C++
//-*-===//
//
...
struct AddAssume : public LoopPass {
  static char ID;
  AddAssume() : LoopPass(ID) {}
...
char AddAssume::ID = 0;
// static RegisterPass<AddAssumePass> X("add-assume", "Add Assume Pass", false,
//                                      false);
INITIALIZE_PASS_BEGIN(AddAssume, "add-assume",
                      "Add Meta Data to Assumptions Pass", false, false)
INITIALIZE_PASS_DEPENDENCY(ScalarEvolutionWrapperPass)
INITIALIZE_PASS_DEPENDENCY(LoopInfoWrapperPass)
INITIALIZE_PASS_END(AddAssume, "add-assume",
                    "Add Meta Data to Assumptions Pass", false, false)

Pass *llvm::createAddAssumePass() { return new AddAssume(); }

// Function to add an assume in the loop preheader
void AddAssume::addAssumeToLoopPreheader(Loop *L, ScalarEvolution *SE,
                                         uint64_t MinIterCount) {
