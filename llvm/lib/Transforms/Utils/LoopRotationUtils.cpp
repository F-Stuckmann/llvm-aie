  ...
  do {
    ...
    // can't duplicate blocks inside it.
    {
      ...
      if (!Metrics.NumInsts.isValid()) {
        LLVM_DEBUG(
            dbgs() << "LoopRotation: NOT rotating - contains instructions"
                      " with invalid cost: ";
            L->dump());
        return Rotated;
      }
    ...
    // OrigHeader.
    if (LoopEntryBranch)
      LLVM_DEBUG(dbgs() << "Removing old Preheader Terminator: ";
                 LoopEntryBranch->dump());
    LoopEntryBranch->eraseFromParent();
    OrigPreheader->flushTerminatorDbgRecords();
    LLVM_DEBUG(dbgs() << "New Preheader:"; OrigPreheader->dump());
    ...
    LLVM_DEBUG(dbgs() << "BranchInst: "; PHBI->dump());
    LLVM_DEBUG(dbgs() << "Condition: "; Cond->dump());
    if (isa<ConstantInt>(Cond)) {
      LLVM_DEBUG(dbgs() << "Condition is constant\n");
    } else {
      LLVM_DEBUG(dbgs() << "Condition is Variable\n");
    }

    // use minimum iteration counts to guide the conditional preheader
    // operation.
    if (L->getLoopPreheader())
      LLVM_DEBUG(dbgs() << "Preheader " << L->getLoopPreheader()->getName());
    else
      LLVM_DEBUG(dbgs() << "loop" << L->getName());
    LLVM_DEBUG(dbgs() << " is conditional?=" << HasConditionalPreHeader
                      << "\n");
...
bool LoopRotate::processLoop(Loop *L) {
  LLVM_DEBUG(dbgs() << "Pre Rotation Dump\n";
  ...
  LLVM_DEBUG(dbgs() << "Processing Loop Rotate: "
  ...
  LLVM_DEBUG(dbgs() << "Post Rotation Dump\n";
             L->getHeader()->getParent()->dump());
