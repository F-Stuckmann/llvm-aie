...
def build_run_list(ti: common.TestInfo, run_lines, verbose=False):
    run_list = []
    all_prefixes = []
    for l in run_lines:
        if "|" not in l:
        ...
        # Apply lit substitutions, including user-specific ones
        for s in ti.lit_substitutions:
            l = l.replace(s[0], s[1])
        ...
        if not llc_cmd.startswith("llc "):
            common.warn("Skipping non-llc RUN line: {}".format(l), test_file=ti.path)
            continue
        if not filecheck_cmd.startswith("FileCheck "):
            common.warn(
                "Skipping non-FileChecked RUN line: {}".format(l), test_file=ti.path
            )
            continue
...
def update_test_file(ti: common.TestInfo):
    args, test, autogenerated_note = ti.args, ti.path, ti.test_autogenerated_note
    ...
    run_list = build_run_list(ti, run_lines, args.verbose)
